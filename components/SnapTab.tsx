...
export default function SnapTab({ profile, onAdd }: { profile: TaxProfile; onAdd: (r: Receipt) => void }) {
  ...
  const [scan, setScan] = useState<ScanState>({ step: 0, stepDone: new Set(), ocrText: '', ocrConfidence: 0, imageData: '', thumb: '', result: null })
  ...
  const markDone = (n: number) => setScan(prev => ({ ...prev, stepDone: new Set([...prev.stepDone, n]) }))

  const processImage = async (dataUrl: string) => {
    setFlow('scanning')
    setError(null)
    setScan({ step: 0, stepDone: new Set(), ocrText: '', ocrConfidence: 0, imageData: dataUrl, thumb: '', result: null })
  ...
  const reset = () => {
    ...
    setScan({ step: 0, stepDone: new Set(), ocrText: '', ocrConfidence: 0, imageData: '', thumb: '', result: null })
      ...
      <div>
        ...
        <div style={{ padding: '28px', maxWidth: 520 }}>
          ...
          <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 20 }}>
            {STEPS.map((label, i) => {
              const id = i + 1
              const done = scan.stepDone.has(id)
